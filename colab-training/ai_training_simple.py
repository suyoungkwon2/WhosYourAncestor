    ...
    for country in countries:
        ...
        for file in files:
            try:
                ...
                img = img.resize((160, 160))  # 더 작은 이미지 사용
...
def train_model_with_augmentation(model, X_train, y_train, X_val, y_val, num_classes):
    """데이터 증강을 사용한 모델 학습"""
    print("🎯 데이터 증강을 사용한 모델 학습 시작...")
...
def main():
    ...
    # 모델 학습
    history = train_model_with_augmentation(model, X_train, y_train, X_val, y_val, num_classes)
    ...
    print("📁 생성된 파일:")
    print("  - best_simple_model.h5 (최고 성능 모델)")
    print("  - web_model_simple/ (TensorFlow.js 모델)")
    print("  - model_info_simple.json (모델 정보)")
    print("  - simple_training_history.png (학습 그래프)")
